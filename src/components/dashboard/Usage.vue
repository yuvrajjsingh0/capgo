...
<template>
  <span class="relative cursor-pointer left-0 top-4 inline-flex items-center rounded-full bg-blue-100 px-3 py-0.5 text-sm font-medium text-blue-800" @click="openPlan()">Beta charts</span>
  <div class="grid md:mx-10" :class=" appId ? 'grid-cols-16 gap-6' : 'gap-6 grid-cols-12 pb-8'">
    <UsageCard v-if="!isLoading" :limits="allLimits.mau" :colors="colors.emerald" :datas="datas.mau" :title="t('MAU')" unit="Users" />
    <div v-else class="flex flex-col h-[425px] bg-white border rounded-sm shadow-lg col-span-full sm:col-span-6 xl:col-span-4 border-slate-200 dark:bg-gray-800 dark:border-slate-900">
    ...
    <div v-else-if="appId" class="flex flex-col h-[425px] bg-white border rounded-sm shadow-lg col-span-full sm:col-span-6 xl:col-span-4 border-slate-200 dark:bg-gray-800 dark:border-slate-900">
      <div class="w-1/2 mx-auto my-auto border-b-2 rounded-full animate-spin aspect-square border-cornflower-600" />
    </div>
  </div>
</template>
